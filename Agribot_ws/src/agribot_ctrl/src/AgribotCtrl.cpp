...
Agribot_ctrl::Agribot_ctrl() {
    theta_initial = 0;
    x_initial = 0;
    y_initial = 0;
}

std_msgs::Float32MultiArray Agribot_ctrl::getLinearSpeed(const geometry_msgs::Twist &cmdVel,float wheel_separation,float dt) {
...
std_msgs::Float32MultiArray Agribot_ctrl::stopAgribot() {
    std_msgs::Float32MultiArray stop;
    stop.data = {0,0};
    return stop;
}
nav_msgs::Odometry Agribot_ctrl::distance_covered_enc(float dt, float w_l, float w_r,float wheel_radius,float wheel_separation,ros::Time current_time) {
    ...
    odom.child_frame_id = "Agribot_base_link";
...
geometry_msgs::Twist Agribot_ctrl::cmdVel(float w_l, float w_r,float wheel_radius,float wheel_separation) {
    matrix1_f << cos(theta_initial), cos(theta_initial),
                 sin(theta_initial), sin(theta_initial),
                 1 / wheel_separation, -1 / wheel_separation;
    matrix2_f << w_r,
